...
test_that("MarginalGenerator$all() different grid", {
  ...
  expect_error({MarginalGenerator$new(grid.dat = grid, dist.dat = mg.dat, features = c("a", "b", "c"))})
  expect_error({MarginalGenerator$new(grid.dat = grid, dist.dat = mg.dat, features = c("a", "c"))})
