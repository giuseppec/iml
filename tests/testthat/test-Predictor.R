...
predictor.S3 = Predictor$new(mod.S3, predict.args = list(type="prob"))
...
predictor.f = Predictor$new(mod.f)
...
test_that("f works", {
  ...
  predictor.f.1 = Predictor$new(mod.f, class = 1)
...
## mlr
predictor.mlr = Predictor$new(mod.mlr, class = 2)
# S3 predict
predictor.S3 = Predictor$new(mod.S3, class = 2, predict.args = list(type="prob"))
# caret
predictor.caret = Predictor$new(mod.caret, class = 2)
# function
predictor.f = Predictor$new(mod.f, class = 2)
prediction.f = predictor.f$predict(iris.test)
test_that("equivalence",{
  expect_equivalent(prediction.f, predictor.caret$predict(iris.test))
  expect_equivalent(predictor.mlr$predict(iris.test), predictor.S3$predict(iris.test))
...
test_that("Missing predict.type gives warning", {
  ...
  mod.mlr = mlr::train(lrn, task)
  expect_warning(Predictor$new(mod.mlr))
})
